...
public class FileDemo {
	...
	public void changeFile(String path) {
		// path => c:\dev\txt\jdk.exe => c:\dev\test\122312313123.exe
		File f1 = new File(path);
		// f1.getPath() = > 전체경로 + 파일이름
		String fileName = f1.getName();// 전체 경로증애 파일이름만 반환
		String name = fileName.substring(0, fileName.lastIndexOf("."));
		String extension = fileName.substring(fileName.lastIndexOf(".") + 1);
		name = String.valueOf(System.currentTimeMillis());
		fileName = name + "." + extension;
		// 파일은 child 폴더 경로 patent
		// f1의 디렉토리를 반한 => c:\dev\test
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * File f2 = new File("c:\\dev\\test", "jdk-11.0.3_windows-x64_bin.exe"); long
		 * fileSize = f2.length() / 1024; System.out.println(f1.getComma(fileSize) +
		 * "KB"); boolean isFile = f2.isFile(); if (isFile) {
		 * 
		 * String path = f2.getPath();// f2의 전체 경로 반환한다.
		 * 
		 * if (path.endsWith("pdf")) { System.out.println("PDF 파일"); } else if
		 * (path.endsWith("exe")) { System.out.println("응용프로그램");
		 * 
		 * } else if (path.endsWith("TXT")) { System.out.println("텍스트문서"); }
		 * 
		 * } else { System.out.println("파일폴더");
		 * 
		 * }
		 * 
		 * File f3 = new File("c:\\dev\\test", "jdk.exe"); f2.renameTo(f3);// f2파일을 f3파일
		 * 이름으로 변경
		 * 
		 * f1.changeFile("c:\\dev\\test\\jdk.exe");
		 */

		File f4 = new File("c:\\dev\\test\\test.html");
